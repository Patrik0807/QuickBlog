...
const CommentTableItem = ({ comment, fetchComments }) => {
    ...
    const unApprovedComment = async () => {
        try {
            const { data } = await axios.post('/api/admin/unapprove-comment', { id: _id })
            if (data.success) {
                toast.success(data.message);
                fetchComments();
            } else {
            ...
            <td className='px-6 py-4 max-sm:hidden'>
                <div className='inline-flex items-center gap-4'>
                    {!comment.isApproved ?
                        ...
                        <p onClick={unApprovedComment} className='text-xs border border-green-600 bg-green-100 text-green-600 rounded-full px-3 py-1 cursor-pointer'>Approved</p>}
