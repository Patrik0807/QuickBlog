...
const BlogTableItem = ({ blog, fetchblogs, index }) => {
    ...
    return (
        <tr className='border-y border-gray-300'>
            ...
            <td className='px-3 py-4 max-sm:hidden'>{BlogDate.toDateString()}</td>
            <td className='px-3 py-4 max-sm:hidden'>
                <p className={`${blog.isPublished ? "text-green-800" : "text-orange-700"}`}>{blog.isPublished ? 'Published' : 'Unpublished'} </p></td>
